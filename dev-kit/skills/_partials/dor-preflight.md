...
Verifies the **structural** subset of an issue's Definition of Ready. This is the canonical source of DoR structural-check logic. Consumer skills (`backlog`, `triage`, `backlog-grooming`) compose this partial rather than re-implementing the same heuristics.

## Modes

The gate has two execution modes, distinguished by who's invoking:

- **Interactive mode** (default; used by the `backlog` skill's Step F.5): WARNING-level misses surface a single warning summary and an explicit `[y/N]` proceed prompt. INFO-level misses are listed but never block. Never silently fixes structure.
- **Non-interactive mode** (used by the `backlog` skill's auto-file mode): runs the same checks and returns a structured pass/fail result to the caller. WARNING-level misses → caller error. The caller is responsible for assembling a conformant body or surfacing the failure.
...
| `needs-grooming` Status label (typically set by the `quick-capture` skill) | **Deferral** — the human author intentionally captured a half-formed idea. | Template-appropriate top-level headings, Type-label, Priority-label, and Epic-linkage checks downgraded to INFO. **User-story check stays WARNING** — it's the one invariant `quick-capture` guarantees, and the only field that distinguishes an honest deferral from a label-bypass. |
...
| Template-appropriate top-level headings | `feature_request.md` requires `## User story` + `## Problem` + `## Proposal` + `## Acceptance criteria`. `tech_debt.md` requires `## User story` + `## Current pain` + `## Motivation` + `## Acceptance criteria`. `bug_report.md` requires `## User story` + `## Problem` + `## Expected vs. actual` + `## Reprex`. `qc_finding.md` requires `## User story` + `## Finding` + `## Acceptance criteria`. `documentation.md` requires `## User story` + `## Gap` + `## Suggested fix` + `## Acceptance criteria`. `other_inquiry.md` requires `## User story` + `## Question` (no Acceptance criteria — a question isn't actionable work). **Downgraded to INFO under `needs-grooming`** (quick-captured bodies use TODO placeholders, which still satisfy the heading-presence check in practice). | WARNING |
| One Type label | At least one of `bug`, `enhancement`, `documentation`, `tech-debt`, `epic`, `audit-finding`, `qc`, `spike` (per [`label-vocabulary.md`](label-vocabulary.md)). **Downgraded to INFO under `needs-grooming`** (quick-capture defaults to `enhancement`; refinement happens at grooming time). | WARNING |
...
| Worth-doing signal | Optional — the `backlog` skill's Step D judgment (now / later / no) was made and, on **now**, a priority label was applied. | INFO |
...
- The `backlog` skill's Step F.5 invokes this in interactive mode.
- The `backlog` skill's auto-file mode invokes this in non-interactive mode.
- The `triage` skill invokes this in non-interactive mode against incoming issues before routing.
- The `backlog-grooming` skill invokes this in interactive mode after groom-inbox fills the deferred sections of a `needs-grooming` issue.

...
| Template-appropriate top-level headings | `feature_request.md` requires `## User story` + `## Problem` + `## Proposal` + `## Acceptance criteria`. `tech_debt.md` requires `## User story` + `## Motivation` + `## Current pain` + `## Acceptance criteria`. `bug_report.md` requires `## User story` + `## Problem` + `## Expected vs. actual` + `## Reprex`. `qc_finding.md` requires `## User story` + `## Finding` + `## Acceptance criteria`. **Downgraded to INFO under `needs-grooming`** (quick-captured bodies use TODO placeholders, which still satisfy the heading-presence check in practice). | WARNING |

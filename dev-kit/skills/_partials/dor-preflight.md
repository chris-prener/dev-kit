...
| One Type label | At least one of `bug`, `enhancement`, `documentation`, `tech-debt`, `epic`, `audit-finding`, `qc` (per [`label-vocabulary.md`](label-vocabulary.md)). **Downgraded to INFO under `needs-grooming`** (quick-capture defaults to `enhancement`; refinement happens at grooming time). | WARNING |
